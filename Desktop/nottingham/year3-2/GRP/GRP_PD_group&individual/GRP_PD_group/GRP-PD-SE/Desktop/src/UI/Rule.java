/**
 * Rule follows Chinese Chess Rule. Written by Sun Qiwei
 */
package UI;
import java.awt.Color;

import javax.swing.*;  
public class Rule {  
    ChessBoard board = null;  
    ChessPiece piece = null;  
    ChessPoint point[][];  
    int startI, startJ, endI, endJ;  
  
    /**
     * 
     * @param board keeping updating
     * @param point record all pieces on the board
     */
    public Rule(ChessBoard board, ChessPoint point[][]) {  
        this.board = board;  
        this.point = point;  
    }  
  
//    /**
//     * Because the engine would calculate the possibility of movements to avoid the King captured, the King would not be captured in the game. 
//     * If no more suggestion, it should give a stop message
//     * 
//     */
//    public void isWin(ChessPiece moveChess,ChessPiece piece) {  
//    	this.piece = piece;  
////        if(piece != null){
////          if (piece.getName() == "将" || piece.getName() == "帅") {  
////            if (piece.getColor() == Color.RED) JOptionPane.showMessageDialog(null, "Black  wins！");
////            else JOptionPane.showMessageDialog(null, "Red  wins！");   
////          }
////        }
//    	
//    }
    

    
    /**
     * 
     * @param piece: the piece needs to move
     * @param startI: point[startI][]
     * @param startJ: point[][startJ]
     * @param endI: point[endI][]
     * @param endJ: point[][endJ]
     * @return
     */
    public boolean movePieceRule(ChessPiece piece, int startI, int startJ, int endI, int endJ) {  
        this.piece = piece;  
        this.startI = startI;  
        this.startJ = startJ;  
        this.endI = endI;  
        this.endJ = endJ;  
        int minI = Math.min(startI, endI);  
        int maxI = Math.max(startI, endI);  
        int minJ = Math.min(startJ, endJ);  
        int maxJ = Math.max(startJ, endJ);  
        boolean isMove = false;  
        if (piece.getName().equals("車")) {  
            if (startI == endI) {  
                int j = 0;  
                for (j = minJ + 1; j <= maxJ - 1; j++) {  
                    if (point[startI][j].isPiece()) {  
                    	isMove = false;  
                        break;  
                    }  
                }  
                if (j == maxJ) {  
                	isMove = true;  
                }  
            } else if (startJ == endJ) {  
                int i = 0;  
                for (i = minI + 1; i <= maxI - 1; i++) {  
                    if (point[i][startJ].isPiece()) {  
                    	isMove = false;  
                        break;  
                    }  
                }  
                if (i == maxI) {  
                	isMove = true;  
                }  
            } else {  
            	isMove = false;  
            }  
  
        } else if (piece.getName().equals("馬")) {  
            int xAxle = Math.abs(startI - endI);  
            int yAxle = Math.abs(startJ - endJ);  
  
            if (xAxle == 2 && yAxle == 1) {  
                if (endI > startI) {  
                    if (point[startI + 1][startJ].isPiece()) {  
                        isMove = false;  
                    } else {  
                        isMove = true;  
                    }  
                }  
                if (endI < startI) {  
                    if (point[startI - 1][startJ].isPiece()) {  
                        isMove = false;  
                    } else {  
                        isMove = true;  
                    }  
                }  
  
            }else if (xAxle == 1 && yAxle == 2) {  
                if (endJ > startJ) {  
                    if (point[startI][startJ + 1].isPiece()) {  
                        isMove = false;  
                    } else {  
                        isMove = true;  
                    }  
                }  
                if (endJ < startJ) {  
                    if (point[startI][startJ - 1].isPiece()) {  
                        isMove = false;  
                    } else {  
                        isMove = true;  
                    }  
                }  
  
            } else {  
                isMove = false;  
            }  
        } else if (piece.getName().equals("象")) {  
            int centerI = (startI + endI) / 2;  
            int centerJ = (startJ + endJ) / 2;  
            int xAxle = Math.abs(startI - endI);  
            int yAxle = Math.abs(startJ - endJ);  
            if (xAxle == 2 && yAxle == 2 && endJ <= 5) {  
                if (point[centerI][centerJ].isPiece()) {  
                    isMove = false;  
                } else {  
                    isMove = true;  
                }  
            } else {  
                isMove = false;  
            }  
        } else if (piece.getName().equals("相")) {  
            int centerI = (startI + endI) / 2;  
            int centerJ = (startJ + endJ) / 2;  
            int xAxle = Math.abs(startI - endI);  
            int yAxle = Math.abs(startJ - endJ);  
            if (xAxle == 2 && yAxle == 2 && endJ >= 6) {  
                if (point[centerI][centerJ].isPiece()) {  
                    isMove = false;  
                } else {  
                    isMove = true;  
                }  
            } else {  
                isMove = false;  
            }  
        } else if (piece.getName().equals("炮")) {  
            int number = 0;  
            if (startI == endI) {  
                int j = 0;  
                for (j = minJ + 1; j <= maxJ - 1; j++) {  
                    if (point[startI][j].isPiece()) {  
                        number++;  
                    }  
                }  
                if (number > 1) {  
                    isMove = false;  
                } else if (number == 1) {  
                    if (point[endI][endJ].isPiece()) {  
                        isMove = true;  
                    }  
                } else if (number == 0 && !point[endI][endJ].isPiece()) {  
                    isMove = true;  
                }  
            } else if (startJ == endJ) {  
                int i = 0;  
                for (i = minI + 1; i <= maxI - 1; i++) {  
                    if (point[i][startJ].isPiece()) {  
                        number++;  
                    }  
                }  
                if (number > 1) {  
                    isMove = false;  
                } else if (number == 1) {  
                    if (point[endI][endJ].isPiece()) {  
                        isMove = true;  
                    }  
                } else if (number == 0 && !point[endI][endJ].isPiece()) {  
                    isMove = true;  
                }  
            } else {  
                isMove = false;  
            }  
        } else if (piece.getName().equals("兵")) {  
            int xAxle = Math.abs(startI - endI);  
            if (endJ >= 6) {  
                if (startJ - endJ == 1 && xAxle == 0) {  
                    isMove = true;  
                }  
  
                else {  
                    isMove = false;  
                }  
            } else if (endJ <= 5) {  
                if ((startJ - endJ == 1) && (xAxle == 0)) {  
                    isMove = true;  
                } else if ((endJ - startJ == 0) && (xAxle == 1)) {  
                    isMove = true;  
                } else {  
                    isMove = false;  
                }  
            }  
        } else if (piece.getName().equals("卒")) {  
            int xAxle = Math.abs(startI - endI);  
            if (endJ <= 5) {  
                if (endJ - startJ == 1 && xAxle == 0) {  
                    isMove = true;  
                } else {  
                    isMove = false;  
                }  
            } else if (endJ >= 6) {  
                if ((endJ - startJ == 1) && (xAxle == 0)) {  
                    isMove = true;  
                } else if ((endJ - startJ == 0) && (xAxle == 1)) {  
                    isMove = true;  
                } else {  
                    isMove = false;  
                }  
            }  
        }  
  
        else if (piece.getName().equals("士")) {  
            int xAxle = Math.abs(startI - endI);  
            int yAxle = Math.abs(startJ - endJ);  
            if (endI <= 6 && endI >= 4 && xAxle == 1 && yAxle == 1) {  
                isMove = true;  
            } else {  
                isMove = false;  
            }  
        } else if (piece.getName().equals("仕")) {  
            int xAxle = Math.abs(startI - endI);  
            int yAxle = Math.abs(startJ - endJ);  
            if (endI <= 6 && endI >= 4 && xAxle == 1 && yAxle == 1) {  
                isMove = true;  
            } else {  
                isMove = false;  
            }  
        } else if ((piece.getName().equals("帅"))  
                || (piece.getName().equals("将"))) {  
            int xAxle = Math.abs(startI - endI);  
            int yAxle = Math.abs(startJ - endJ);  
            if (endI <= 6 && endI >= 4) {  
                if ((xAxle == 1 && yAxle == 0) || (xAxle == 0 && yAxle == 1)) {  
                    isMove = true;  
                } else {  
                    isMove = false;  
                }  
            } else {  
                isMove = false;  
            }  
        }  
  
        return isMove;  
  
    }  
}