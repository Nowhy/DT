...
public class SearchEngine {
	...
	// 5. 搜索结果
	public int lastScore, pvLineNum;
...
/*	
	public MoveNode[] getBestMoveTrack() throws LostException{
		control();
		MoveNode[] track = new MoveNode[5];
		for(int i = 0; i < 5;i++){
			track[i]=hashList[i].bestMove;
		}
		return track;
	}
	...
	public MoveNode[] getBestMoveTrack() throws LostException{
		control();
		MoveNode[] tracks = new MoveNode[bookList.length];
		for(int i = 0; i < bookList.length; i++){
			tracks[i] = bookList[i].bestMove;
		}
