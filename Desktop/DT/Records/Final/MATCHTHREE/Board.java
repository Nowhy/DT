...
public class Board {
		...
		for(int i = 0; i < BOARDSIZE; i++) {
			for(int j = 0; j < BOARDSIZE; j++) {
				...
				if (oldcandy instanceof RegularCandy){
					Candy newcandy = new RegularCandy(oldcandy.colour, this, i, j);
					this.candies[i][j] = newcandy;
				}else if(oldcandy instanceof XCandy) {
					Candy newcandy = new XCandy(oldcandy.colour, this, i, j);
					this.candies[i][j] = newcandy;
					this.setFourXNUM(this.getFourXNUM()+1);
				}else if(oldcandy instanceof YCandy) {
					Candy newcandy = new YCandy(oldcandy.colour, this, i, j);
					this.candies[i][j] = newcandy;
					this.setFourYNUM(this.getFourYNUM()+1);
				}else if(oldcandy instanceof BombCandy) {
					Candy newcandy = new BombCandy(oldcandy.colour, this, i, j);
					this.candies[i][j] = newcandy;
					this.setFiveNUM(this.getFiveNUM()+1);
				}else if(oldcandy instanceof ChocolateCandy) {
					Candy newcandy = new ChocolateCandy(-1, this, i, j);
					this.candies[i][j] = newcandy;
					this.setChocolateNUM(this.getChocolateNUM()+1);
	...
	public Board(Game game, int LEVEL, int Player){ 
		this.game = game;
...
//					mark = 0;
//				}
//				int colour = random[mark];
//				mark ++;
//				candy = new RegularCandy(colour, this, i, j);
//			}else if(METHOD == 2) {
	...
	public boolean checkValidMoves(){
		for(int i = BOARDSIZE-1; i >= 0; i--) {
			for(int j = BOARDSIZE-1; j >= 0; j--) {
				if(this.getCandies()[i][j] != null && this.getCandies()[i][j].getcolour()!=-10) {
					this.setFirstCandy(i, j);
					this.setSelectedFirst();
					if(i > 0 && this.getCandies()[i-1][j] != null && this.getCandies()[i-1][j].getcolour()!=-10) {
						Candy SecondCandy = this.getCandies()[i-1][j];
						if(isValid(FirstCandy, SecondCandy)) return true;
					}
					if(j > 0 && this.getCandies()[i][j-1] != null && this.getCandies()[i][j-1].getcolour()!=-10) {
...
//		System.out.println(FirstCandy.colour + "  11111111111            " + SecondCandy.colour);
...
//			System.out.println("two special candy");
	...
	public void Bomb(Candy[][] candies, Candy FirstCandy, Candy SecondCandy) {
		if ((FirstCandy instanceof XCandy && SecondCandy instanceof XCandy) || (FirstCandy instanceof XCandy && SecondCandy instanceof YCandy) || (FirstCandy instanceof YCandy && SecondCandy instanceof YCandy) || (FirstCandy instanceof YCandy && SecondCandy instanceof XCandy)) {
			int firstcolour = FirstCandy.getcolour();
			int firstRow = FirstCandy.getRow();
			int firstColumn = FirstCandy.getColumn();
			Candy newFirst = new XCandy(firstcolour, this, firstRow, firstColumn);
			...
			int secondcolour = SecondCandy.getcolour();
			int secondRow = SecondCandy.getRow();
			int secondColumn = SecondCandy.getColumn();
			Candy newSecond = new YCandy(secondcolour, this, secondRow, secondColumn);
			...
			if((FirstCandy instanceof BombCandy && SecondCandy instanceof XCandy) || (FirstCandy instanceof XCandy && SecondCandy instanceof BombCandy)) {
				this.setFourXNUM(this.getFourXNUM()+2);
			}else {
				this.setFourYNUM(this.getFourYNUM()+2);
			}
			...
			this.setFiveNUM(this.getFiveNUM()+2);
			int row1 = FirstCandy.getRow();
			int column1 = FirstCandy.getColumn();
			int row2 = SecondCandy.getRow();
			int column2 = SecondCandy.getColumn();
	...
	public boolean isValid(Candy FirstCandy, Candy SecondCandy) {
		...
		if(FirstCandy.colour == -10 || SecondCandy.colour == -10) {
			return false;
		}
		int row1 = FirstCandy.getRow();
		int column1 = FirstCandy.getColumn();
		...
		if(!mark || (FirstCandy.getcolour() == SecondCandy.getcolour())) {
			return false;
		}
			...
			for (int j = BOARDSIZE - 1;j >= 0;j--){
				if(candies[i][j]==null){
					if(i == 0) {
						...
						if(candies[i-1][j] != null && candies[i-1][j].getcolour() != -10){
							moveCandyTo(i-1,j,i,j);
						}else if(candies[i-1][j] != null && candies[i-1][j].getcolour() == -10){
							int m = i-1;
							while(m >= 0) {
								if(candies[m][j] != null && candies[m][j].getcolour() != -10) {
									moveCandyTo(m,j,i,j);
									break;
								}
								m--;
...
//		System.out.println(FirstCandy.colour + "       " +  FirstCandy.getRow() + "       " + FirstCandy.getColumn());
	...
	public void printBoard(){
		...
		for (int i = 0; i < BOARDSIZE; i++){
			...
			for (int j = 0; j < BOARDSIZE; j++){
				...
				else if(candies[i][j].getcolour() != -10) {
					System.out.print( "    " + candies[i][j].getcolour());
				}else {
					System.out.print( "  " + candies[i][j].getcolour());
				}
			}
